...
public class BluetoothChatService {
	...
	public synchronized void connected(BluetoothSocket socket,
		...
		ConnectedThread _mConnectedThread = new ConnectedThread(socket,
				socketType);
	...
	public void write(byte[] out) {
		...
		mHandler.obtainMessage(BluetoothChat.MESSAGE_WRITE, -1, -1, out)
				.sendToTarget();
	...
	public class ConnectedThread extends Thread {
				...
				try {
					...
					if (fileMode) {
						...
						if (fileSize >= totalSize) {
							...
							Date d = new Date();
							String date = BluetoothChat.pad(d.getHours()) + ":"
									+ BluetoothChat.pad(d.getMinutes()) + ":"
									+ BluetoothChat.pad(d.getSeconds());

							BluetoothMessage bm = new BluetoothMessage(
									fileAuthor, date, "file " + fileName
											+ " sent");

							mHandler.obtainMessage(BluetoothChat.MESSAGE_READ,
									bytes, -1, bm).sendToTarget();

							Uri uri = Uri.fromFile(savedFile);
							if (bluetoothClients.size() > 1) {
								for (int i = 0; i < bluetoothClients.size(); i++) {
									if (!bluetoothClients.get(i).deviceAdress
											.equals(mmSocket.getRemoteDevice()
													.getAddress())) {
										bluetoothClients.get(i).connectedThread
												.writeFile(uri);
									}
								}
							}
		...
		public void writeFile(Uri uri) throws IOException, InterruptedException {
			...
			InputStream inputStream = bc.getContentResolver().openInputStream(
					uri);

			long fileSize = inputStream.available();

			BluetoothMessage m = new BluetoothMessage(BluetoothAdapter
					.getDefaultAdapter().getName(), String.valueOf(fileSize),
					fileName, BluetoothMessage.TYPE_FILE_START);
			write(m.getJSONStr());
